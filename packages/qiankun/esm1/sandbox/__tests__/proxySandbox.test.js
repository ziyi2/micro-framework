...
it('eval should never be represented', () => {
    ...
    // @ts-ignore
    window.proxy = proxy;
    const code = ';(function(window){with(window){ var testProp = function(wrequire){ eval("window.testEval=wrequire()"); }; testProp(() => "kuitos");}})(window.proxy)';
...
it('descriptor of non-configurable and non-enumerable property existed in raw window should be the same after modified in sandbox', () => {
    ...
    const { proxy } = new ProxySandbox('unit-test');
    proxy.nonConfigurableProp = window.nonConfigurableProp;
    proxy.nonConfigurablePropWithAccessor = 123;
    expect(proxy.nonConfigurablePropWithAccessor).toBe(undefined);
    expect(Object.keys(proxy)).toEqual(Object.keys(window));
    expect(Object.getOwnPropertyDescriptor(proxy, 'nonConfigurableProp')).toEqual(Object.getOwnPropertyDescriptor(window, 'nonConfigurableProp'));
...
it('hasOwnProperty should always returns same reference', () => {
    const proxy = new ProxySandbox('hasOwnProperty-test').proxy;
    proxy.testA = {};
    proxy.testB = {};
...
it('native window function calling should always be bound with window', () => {
    window.mockNativeWindowFunction = function mockNativeWindowFunction() {
...
it('native bounded function should not be rebounded', () => {
    const proxy = new ProxySandbox('bound-native-fn-test').proxy;
...
it('non-native function should not be rebounded', () => {
    const proxy = new ProxySandbox('non-native-fn-bound-test').proxy;
    function test() { }
    proxy.fn = test;
...
it('frozen property should not be overwrite', () => {
    ...
    const proxy = new ProxySandbox('bound-fn-test', globalContext).proxy;
...
it('the prototype should be kept while we create a function with prototype on proxy', () => {
    const proxy = new ProxySandbox('new-function').proxy;
...
it('some native window property was defined with getter in safari and firefox, and they will check the caller source', () => {
    ...
    expect(window.mockSafariGetterProperty).toBe('getterPropertyInSafariWindow');
    const { proxy } = new ProxySandbox('object-define-property-target-test');
...
describe('should work with nest sandbox', () => {
    it('specified dom api should bound with native window', () => {
        ...
        const { proxy: nestProxy } = new ProxySandbox('dom-api', sandboxProxy);
        function mockDomAPI() {
            if (this !== window) {
                throw new TypeError('Illegal invocation!');
            }
        ...
        nestProxy.mockDomAPINotInBlackList = function fnCp() {
