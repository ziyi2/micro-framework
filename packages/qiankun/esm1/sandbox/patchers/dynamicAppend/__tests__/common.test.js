...
const createStyleElement = () => {
    ...
    return document.getElementById('style-under-test');
};
beforeEach(() => {
...
test('should record Styled-Component CSS Rules correctly', () => {
    ...
    const cssRules = getStyledElementCSSRules(styleElement);
    expect(cssRules).toBeDefined();
    expect(cssRules?.length).toEqual(2);
    expect((cssStyleSheet?.cssRules[0]).selectorText).toEqual('span');
    expect((cssStyleSheet?.cssRules[1]).selectorText).toEqual('#foo');
});
