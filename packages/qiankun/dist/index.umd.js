(function (global, factory) {
	...
	function _getExternalStyleSheets(styles) {
	  ...
	  return allSettledButCanBreak(styles.map( /*#__PURE__*/function () {
	    var _ref = _asyncToGenerator$1( /*#__PURE__*/regenerator.mark(function _callee(styleLink) {
	...
	function _getExternalScripts(scripts) {
	  ...
	  return allSettledButCanBreak(scripts.map( /*#__PURE__*/function () {
	    var _ref2 = _asyncToGenerator$1( /*#__PURE__*/regenerator.mark(function _callee2(script) {
	...
	function getAddOns(global, publicPath) {
	  ...
	  return mergeWith_1({}, getAddOn(global), getAddOn$1(global, publicPath), (v1, v2) =>
	  ...
	  concat_1(v1 !== null && v1 !== void 0 ? v1 : [], v2 !== null && v2 !== void 0 ? v2 : []));
	}
	...
	function initGlobalState(state = {}) {
	  ...
	  if (state === globalState) {
	    ...
	    const prevGlobalState = cloneDeep_1(globalState);
	...
	function getMicroAppStateActions(id, isMaster) {
	  return {
	    ...
	    onGlobalStateChange(callback, fireImmediately) {
	      ...
	      deps[id] = callback;
	      if (fireImmediately) {
	        const cloneState = cloneDeep_1(globalState);
	    ...
	    setGlobalState(state = {}) {
	      ...
	      const changeKeys = [];
	      const prevGlobalState = cloneDeep_1(globalState);
	      globalState = cloneDeep_1(Object.keys(state).reduce((_globalState, changeKey) => {
	...
	const isConstDestructAssignmentSupported = memoize_1(() => {
	  try {
	...
	// name: 子应用的 appInstanceId
	// sandboxOpts: 沙箱配置
	function getDefaultTplWrapper(name, sandboxOpts) {
	  // tpl: import-html-entry 生成的模板
	  return tpl => {
	    ...
	    if (tpl.indexOf('<head>') !== -1) {
	      ...
	      // 需要模拟一个 head 占位符，因为原生 head 元素将在微应用中被浏览器擦除
	      ...
	      // 有些模板可能不是标准的 html 文档，因此我们需要为它们添加一个模拟的 head 标签
	      tplWithSimulatedHead = `<${qiankunHeadTagName}></${qiankunHeadTagName}>${tpl}`;
	    }
	    // 将模板包裹在一个 div 中，并设置 id、data-name、data-version、data-sandbox-cfg 属性
	...
	// 获取子应用包裹器的 id
	// 例如：__qiankun_microapp_wrapper_for_vue__
	function getWrapperId(name) {
	  return `__qiankun_microapp_wrapper_for_${snakeCase_1(name)}__`;
	}
	const nativeGlobal = new Function('return this')();
	const nativeDocument = new Function('return document')();
	const getGlobalAppInstanceMap = once_1(() => {
	...
	function validateExportLifecycle(exports) {
	  const _ref = exports !== null && exports !== void 0 ? exports : {},
	...
	const supportsUserTiming$1 = typeof performance !== 'undefined' && typeof performance.mark === 'function' && typeof performance.clearMarks === 'function' && typeof performance.measure === 'function' && typeof performance.clearMeasures === 'function' && typeof performance.getEntriesByName === 'function';
	...
	function getContainer(container) {
	  // 如果 container 是字符串，则返回 document.querySelector(container)，否则返回 container
	...
	info(LogPrefix.PROXY_SANDBOX, `[globalVariableWhiteList](沙箱逃逸的全局变量): ${JSON.stringify(globalVariableWhiteList, null, 2)}`);
	...
	const cachedGlobals = Array.from(new Set(without_1(globalsInES2015.concat(overwrittenGlobals).concat('requestAnimationFrame'), ...accessingSpiedGlobals)));
	    ...
	    const proxy = new Proxy(fakeWindow, {
	      set: (target, p, value) => {
	        ...
	        if (this.sandboxRunning) {
	          ...
	          if (typeof p === 'string' && globalVariableWhiteList.indexOf(p) !== -1) {
	            info(LogPrefix.PROXY_SANDBOX + `[沙箱逃逸][set][${p}] ${p} 在 globalVariableWhiteList 逃逸的白名单中，设置 globalContext["${p}"] =`, value);
	            // 记录白名单属性的原始描述符
	            ...
	            if (!target.hasOwnProperty(p) && globalContext.hasOwnProperty(p)) {
	              ...
	              if (writable || set) {
	                info(LogPrefix.PROXY_SANDBOX + `[沙箱捕获][set][${sp}] ${sp} 不存在隔离的 fakeWindow 对象中，但存在于 globalContext 中，并且 ${sp} 可读可 set，设置 fakeWindow["${sp}"] =`,
	              ...
	              info(LogPrefix.PROXY_SANDBOX +
	              // eslint-disable-next-line max-len
	              `[沙箱捕获][set][${sp}] ${sp} 存在于隔离的 fakeWindow 对象中或者 ${sp} 不存在于 globalContext 中，设置 fakeWindow["${sp}"] =`, value);
	              // 将值设置到 fakeWindow 上
	              target[p] = value;
	            }
	          }
	      ...
	      get: (target, p) => {
	        ...
	        if (p === 'string' && globalVariableWhiteList.indexOf(p) !== -1) {
	          info(LogPrefix.PROXY_SANDBOX, `[沙箱逃逸][get][${p}] 使用 globalContext 对象访问 globalVariableWhiteList 沙箱逃逸白名单 ${p}, 获取 globalContext["${p}"]`);
	          // @ts-ignore
	          return globalContext[p];
	        }
	        ...
	        */
	        ...
	        info(LogPrefix.PROXY_SANDBOX, `[沙箱逃逸][get][${sp}] 访问需要绑定到原生 window 对象的属性，获取 ${sandboxInfo}`);
	      ...
	      defineProperty: (target, p, attributes) => {
	        ...
	        switch (from) {
	          case 'globalContext':
	            ...
	            info(LogPrefix.PROXY_SANDBOX, `[defineProperty] 通过 globalContext 来定义 ${sp} 的描述符对象 ${attributes}`);
	...
	function calcAppCount(appName, calcType, status) {
	  const appCount = appsCounterMap.get(appName) || {
	...
	function isAllAppsUnmounted() {
	  return Array.from(appsCounterMap.entries()).every(([, {
	    bootstrappingPatchCount: bpc,
	    mountingPatchCount: mpc
	  }]) => bpc === 0 && mpc === 0);
	}
	...
	function getOverwrittenAppendChildOrInsertBefore(opts) {
	  function appendChildOrInsertBefore(newChild, refChild = null) {
	    ...
	    const rawDOMAppendOrInsertBefore = opts.rawDOMAppendOrInsertBefore,
	    ...
	    if (element.tagName) {
	      ...
	      const appName = containerConfig.appName,
	        ...
	        case STYLE_TAG_NAME:
	          {
	            ...
	            const _stylesheetElement = stylesheetElement,
	            ...
	            if (scopedCSS) {
	              ...
	              const linkElementUsingStylesheet = ((_element$tagName = element.tagName) === null || _element$tagName === void 0 ? void 0 : _element$tagName.toUpperCase()) === LINK_TAG_NAME && element.rel === 'stylesheet' && element.href;
	              if (linkElementUsingStylesheet) {
	                var _frameworkConfigurati;
	                const fetch = typeof frameworkConfiguration.fetch === 'function' ? frameworkConfiguration.fetch : (_frameworkConfigurati = frameworkConfiguration.fetch) === null || _frameworkConfigurati === void 0 ? void 0 : _frameworkConfigurati.fn;
	                stylesheetElement = convertLinkAsStyle(element, styleElement => process(appWrapper, styleElement, appName), fetch);
	        ...
	        case SCRIPT_TAG_NAME:
	          {
	            ...
	            const appWrapper = appWrapperGetter();
	            const mountDOM = target === 'head' ? getAppWrapperHeadElement(appWrapper) : appWrapper;
	            const fetch = frameworkConfiguration.fetch;
	            const referenceNode = mountDOM.contains(refChild) ? refChild : null;
	            ...
	            _execScripts(null, [`<script>${text}</script>`], proxy, {
	              strictGlobal,
	              scopedGlobalVariables
	            });
	...
	function getNewRemoveChild(rawRemoveChild, containerConfigGetter, target, isInvokedByMicroApp) {
	  function removeChild(child) {
	    ...
	    try {
	      let attachedElement;
	      const _containerConfigGette = containerConfigGetter(child),
	...
	function patchLooseSandbox(appName, appWrapperGetter, sandbox, mounting = true, scopedCSS = false, excludeAssetFilter) {
	   ...
	   */
	  () => Tt(window.location).some(name => name === appName), () => ({
	    appName,
	    appWrapperGetter,
	    proxy,
	    strictGlobal: false,
	...
	function patchDocument(cfg) {
	  ...
	  if (speedy) {
	    ...
	    if (parentNodeDescriptor && !patchMap.has(parentNodeDescriptor)) {
	      const parentNodeGetter = parentNodeDescriptor.get,
	        configurable = parentNodeDescriptor.configurable;
	      if (parentNodeGetter && configurable) {
	        const patchedParentNodeDescriptor = _objectSpread(_objectSpread({}, parentNodeDescriptor), {}, {
	          get() {
	            const parentNode = parentNodeGetter.call(this);
	            if (parentNode instanceof Document) {
	              var _getCurrentRunningApp;
	              const proxy = (_getCurrentRunningApp = getCurrentRunningApp()) === null || _getCurrentRunningApp === void 0 ? void 0 : _getCurrentRunningApp.window;
	              if (proxy) {
	...
	function patchStrictSandbox(appName, appWrapperGetter, sandbox, mounting = true, scopedCSS = false, excludeAssetFilter, speedySandbox = false) {
	  ...
	  const _containerConfig = containerConfig,
	    dynamicStyleSheetElements = _containerConfig.dynamicStyleSheetElements;
	  const unpatchDynamicAppendPrototypeFunctions = patchHTMLDynamicAppendPrototypeFunctions(element => elementAttachContainerConfigMap.has(element), element => elementAttachContainerConfigMap.get(element));
	...
	function patchAtMounting(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter, speedySandBox) {
	  ...
	  const basePatchers = [() => patch$1(sandbox.proxy), () => patch$2(sandbox.proxy), () => patch()];
	  const patchersInSandbox = {
	    [exports.SandBoxType.LegacyProxy]: [...basePatchers, () => patchLooseSandbox(appName, elementGetter, sandbox, true, scopedCSS, excludeAssetFilter)],
	    [exports.SandBoxType.Proxy]: [...basePatchers, () => patchStrictSandbox(appName, elementGetter, sandbox, true, scopedCSS, excludeAssetFilter, speedySandBox)],
	    [exports.SandBoxType.Snapshot]: [...basePatchers, () => patchLooseSandbox(appName, elementGetter, sandbox, true, scopedCSS, excludeAssetFilter)]
	  };
	  return (_patchersInSandbox$sa = patchersInSandbox[sandbox.type]) === null || _patchersInSandbox$sa === void 0 ? void 0 : _patchersInSandbox$sa.map(patch => patch());
	}
	function patchAtBootstrapping(appName, elementGetter, sandbox, scopedCSS, excludeAssetFilter, speedySandBox) {
	  ...
	  const patchersInSandbox = {
	    [exports.SandBoxType.LegacyProxy]: [() => patchLooseSandbox(appName, elementGetter, sandbox, false, scopedCSS, excludeAssetFilter)],
	    [exports.SandBoxType.Proxy]: [() => patchStrictSandbox(appName, elementGetter, sandbox, false, scopedCSS, excludeAssetFilter, speedySandBox)],
	    [exports.SandBoxType.Snapshot]: [() => patchLooseSandbox(appName, elementGetter, sandbox, false, scopedCSS, excludeAssetFilter)]
	  };
	...
	const _excluded = ["singular", "sandbox", "excludeAssetFilter", "globalContext"];
	...
	function _validateSingularMode() {
	  _validateSingularMode = _asyncToGenerator(function* (validate, app) {
	...
	function createElement(appContent, strictStyleIsolation, scopedCSS, appInstanceId) {
	  ...
	  if (scopedCSS) {
	    ...
	    // 遍历内联样式节点
	    forEach_1(styleNodes, stylesheetElement => {
	...
	function _loadApp() {
	  _loadApp = _asyncToGenerator(function* (app, configuration = {}, lifeCycles) {
	    ...
	    const _configuration$singul = configuration.singular,
	      ...
	      importEntryOpts = _objectWithoutProperties(configuration, _excluded);
	    // get the entry html content and script executor
	    // 获取微应用的模板、执行脚本、资源公共路径、获取外部脚本等
	    const _yield$importEntry = yield importEntry(entry, importEntryOpts),
	    ...
	    // 等待微应用的外部脚本加载完成
	    yield getExternalScripts();
	    // as single-spa load and bootstrap new app parallel with other apps unmounting
	    // (see https://github.com/CanopyTax/single-spa/blob/master/src/navigation/reroute.js#L74)
	    // we need wait to load the app until all apps are finishing unmount in singular mode
	    // 如果是单实例模式，需要等待上一个应用卸载完成后再加载当前应用
	    if (yield validateSingularMode(singular, app)) {
	      yield prevAppUnmountedDeferred && prevAppUnmountedDeferred.promise;
	    }
	    // 入参说明：
	    ...
	    if (sandbox) {
	      ...
	      // 是否启用了 Scoped 样式隔离
	      scopedCSS, useLooseSandbox, excludeAssetFilter,
	      // 传入全局对象
	      global, speedySandbox);
	    ...
	    const _mergeWith = mergeWith_1({}, getAddOns(global, assetPublicPath), lifeCycles, (v1, v2) => concat_1(v1 !== null && v1 !== void 0 ? v1 : [], v2 !== null && v2 !== void 0 ? v2 : [])),
	    ...
	    yield execHooksChain(toArray(beforeLoad), app, global);
	    // get the lifecycle hooks from module exports
	    // 执行微应用的 entry 脚本，获取微应用的导出对象
	    const scriptExports = yield execScripts(global, sandbox && !useLooseSandbox, {
	      scopedGlobalVariables: speedySandbox ? cachedGlobals : []
	    });
	    // 获取微应用的生命周期函数
	    const _getLifecyclesFromExp = getLifecyclesFromExports(scriptExports,
	      // 微应用的名称，例如 vue
	      appName,
	      // 全局对象
	      global, // 无沙箱模式下为 undefined
	      (_sandboxContainer = sandboxContainer) === null || _sandboxContainer === void 0 ? void 0 : (_sandboxContainer$ins = _sandboxContainer.instance) === null || _sandboxContainer$ins === void 0 ? void 0 : _sandboxContainer$ins.latestSetProp),
	    ...
	    // 暂时忽略，后续讲解
	    // 用于主应用和微应用之间通信
	    const _getMicroAppStateActi = getMicroAppStateActions(appInstanceId),
	    ...
	    const parcelConfigGetter = (remountContainer = initialContainer) => {
	      ...
	      const parcelConfig = {
	        ...
	        mount: [/*#__PURE__*/_asyncToGenerator(function* () {
	          ...
	          if ((yield validateSingularMode(singular, app)) && prevAppUnmountedDeferred) {
	        ...
	        // 执行 beforeMount 钩子函数
	        _asyncToGenerator(function* () {
	          return execHooksChain(toArray(beforeMount), app, global);
	        }), (
	        /*#__PURE__*/
	        ...
	        function () {
	          var _ref6 = _asyncToGenerator(function* (props) {
	            return mount(_objectSpread(_objectSpread({}, props), {}, {
	        ...
	        // 执行 afterMount 钩子函数
	        _asyncToGenerator(function* () {
	          return execHooksChain(toArray(afterMount), app, global);
	        }),
	        /*#__PURE__*/
	        ...
	        _asyncToGenerator(function* () {
	          return execHooksChain(toArray(beforeUnmount), app, global);
	        }), (
	        /*#__PURE__*/
	        ...
	        function () {
	          var _ref12 = _asyncToGenerator(function* (props) {
	            return unmount(_objectSpread(_objectSpread({}, props), {}, {
	              container: appWrapperGetter()
	            }));
	          });
	          return function (_x5) {
	        ...
	        _asyncToGenerator(function* () {
	          return execHooksChain(toArray(afterUnmount), app, global);
	        }), /*#__PURE__*/_asyncToGenerator(function* () {
	...
	if (typeof window.requestIdleCallback !== 'undefined') {
	  ...
	  requestIdleCallback$1 = cb => setTimeout(idleCall, 0, cb, Date.now());
	}
	// 检测是否为慢网络
	...
	function prefetch(entry, opts) {
	  ...
	  requestIdleCallback$1( /*#__PURE__*/_asyncToGenerator(function* () {
	    // 通过 import-html-entry 来加载微应用的资源
	    const _yield$importEntry = yield importEntry(entry, opts),
	      getExternalScripts = _yield$importEntry.getExternalScripts,
	...
	function prefetchAfterFirstMounted(apps, opts) {
	  // 监听 single-spa:first-mount 事件，等第一个微应用挂载完成后再预加载其他微应用
	...
	function doPrefetchStrategy(apps, prefetchStrategy, importEntryOpts) {
	  ...
	  if (Array.isArray(prefetchStrategy)) {
	    ...
	    _asyncToGenerator(function* () {
	      ...
	      const _yield$prefetchStrate = yield prefetchStrategy(apps),
	    ...
	    switch (prefetchStrategy) {
	      // 调用 qiankun 的 start 方法后，会加载第一个微应用，所以这里等待第一个微应用加载完成后再预加载其他微应用
	...
	const _excluded$1 = ["name", "activeRule", "loader", "props"],
	  _excluded2 = ["mount"],
	...
	const autoDowngradeForLowVersionBrowser = configuration => {
	  ...
	  const _configuration$sandbo = configuration.sandbox,
	    sandbox = _configuration$sandbo === void 0 ? true : _configuration$sandbo,
	  ...
	  if (sandbox) {
	    ...
	    if (!window.Proxy) {
	      ...
	      return _objectSpread(_objectSpread({}, configuration), {}, {
	        sandbox: typeof sandbox === 'object' ? _objectSpread(_objectSpread({}, sandbox), {}, {
	          loose: true
	        }) : {
	          loose: true
	        }
	    ...
	    !isConstDestructAssignmentSupported() && (sandbox === true || typeof sandbox === 'object' && sandbox.speedy !== false)) {
	...
	function registerMicroApps(apps, lifeCycles) {
	  ...
	  unregisteredApps.forEach(app => {
	    ...
	    bt({
	      ...
	      app: function () {
	        var _app = _asyncToGenerator(function* () {
	          ...
	          return _objectSpread({
	            mount: [/*#__PURE__*/_asyncToGenerator(function* () {
	              return loader(true);
	            }), ...toArray(mount), /*#__PURE__*/_asyncToGenerator(function* () {
	              return loader(false);
	            })]
	...
	function loadMicroApp(app, configuration, lifeCycles) {
	    ...
	    if (container) {
	      if (containerXPath) {
	        ...
	        if (containerMicroApps === null || containerMicroApps === void 0 ? void 0 : containerMicroApps.length) {
	          const mount = [/*#__PURE__*/_asyncToGenerator(function* () {
	            // While there are multiple micro apps mounted on the same container, we must wait until the prev instances all had unmounted
	            // Otherwise it will lead some concurrent issues
	            const prevLoadMicroApps = containerMicroApps.slice(0, containerMicroApps.indexOf(microApp));
	            const prevLoadMicroAppsWhichNotBroken = prevLoadMicroApps.filter(v => v.getStatus() !== 'LOAD_ERROR' && v.getStatus() !== 'SKIP_BECAUSE_BROKEN');
	            yield Promise.all(prevLoadMicroAppsWhichNotBroken.map(v => v.unmountPromise));
	   ...
	   */
	  const memorizedLoadingFn = /*#__PURE__*/function () {
	    var _ref4 = _asyncToGenerator(function* () {
	      const userConfiguration = autoDowngradeForLowVersionBrowser(configuration !== null && configuration !== void 0 ? configuration : _objectSpread(_objectSpread({}, frameworkConfiguration), {}, {
	        singular: false
	      }));
	      const $$cacheLifecycleByAppName = userConfiguration.$$cacheLifecycleByAppName;
	      if (container) {
	        // using appName as cache for internal experimental scenario
	        if ($$cacheLifecycleByAppName) {
	          const parcelConfigGetterPromise = appConfigPromiseGetterMap.get(name);
	          if (parcelConfigGetterPromise) return wrapParcelConfigForRemount((yield parcelConfigGetterPromise)(container));
	        }
	        if (containerXPath) {
	      ...
	      return (yield parcelConfigObjectGetterPromise)(container);
	    });
	...
	function start(opts = {}) {
	  ...
	  const _frameworkConfigurati2 = frameworkConfiguration,
