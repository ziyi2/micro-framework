...
class Build extends Base {
  ...
  buildFlatFiles(files: string[], destPath: string) {
    ...
    files.forEach((file) => {
      ...
      // 转换引入路径，例如: import('./core/core') => import('./core')
      code = code.replace(/(?<=(require|import)\(")(.*?)(?="\))/g, (match) =>
        this.replaceImportPath(match, file)
      );

      // 转换引入路径，例如: import core from './core/core' => import core from './core'
      code = code.replace(
        /(?<=((import|export) .* from "))(.*?)(?=")/g,
        (match) => this.replaceImportPath(match, file)
      );
  ...
  // 识别引入模块的路径
  replaceImportPath(match: string, file: string): string {
    if (!match) {
      return match;
    }
    // 例如： match = './core/core'
    ...
    // 例如：引入库包的情况，例如 require("lodash")
    // 例如：引入单个相对路径的情况，例如 require("./core")
    if (paths.length === 1) {
      return match;
    }

    // 例如：库包按需引入的情况，例如 require("lodash/clone")
    if (!paths[0].includes(".")) {
      return match;
    }
